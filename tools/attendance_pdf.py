...
def create_attendance_pdf(report_date, rows, output_path):
    ...
    departments = sorted(
        grouped,
        key=lambda department: (department == review_department, department),
    )
    if not departments:
    ...
    for department_index, department in enumerate(departments):
        ...
        if department == review_department:
            ...
            pdf.drawString(
                48,
                page_height - 105,
                "These employees were excluded from attendance calculations. Review them in UKG.",
            )
            y = page_height - 134
            for row in grouped[department]:
                pdf.drawString(58, y, row["employee"])
                y -= 18
            continue
