...
def create_attendance_pdf(report_date, rows, output_path):
    ...
    def table_header(current_y):
        ...
        for x, label in zip(
            (48, 220, 286, 342, 414, 474),
            ("Employee", "Date", "Late", "Left Early", "Lunch", "Status"),
        ):
    ...
    for department_index, department in enumerate(departments):
        ...
        if not department_rows:
            ...
            for row_index, row in enumerate(department_rows):
                ...
                pdf.drawString(220, y, row["date"].strftime("%m/%d/%Y"))
                pdf.drawString(286, y, _format_minutes(row["late_minutes"]))
                pdf.drawString(342, y, _format_minutes(row["early_minutes"]))
                pdf.drawString(414, y, row["lunch"])
                pdf.drawString(474, y, row.get("status", "-"))
                y -= 20
