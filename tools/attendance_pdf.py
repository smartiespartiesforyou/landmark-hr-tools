...
def create_attendance_pdf(report_date, rows, output_path):
    ...
    def table_header(current_y):
        ...
        for x, label in zip(
            (48, 250, 320, 390, 475),
            ("Employee", "Date", "Late", "Left Early", "Lunch"),
        ):
    ...
    def footer():
        ...
        pdf.drawString(
            42,
            34,
            "Only attendance exceptions are shown. A 7-minute grace period is applied.",
        )
    ...
    for department_index, department in enumerate(departments):
        ...
        if not department_rows:
            ...
            for row_index, row in enumerate(department_rows):
                ...
                pdf.drawString(250, y, row["date"].strftime("%m/%d/%Y"))
                pdf.drawString(320, y, _format_minutes(row["late_minutes"]))
                pdf.drawString(390, y, _format_minutes(row["early_minutes"]))
                pdf.drawString(475, y, row["lunch"])
                y -= 20
